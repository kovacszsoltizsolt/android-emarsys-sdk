...
for i in $MODULES; do
   gcloud firebase test android run --type instrumentation --test "$i/build/outputs/apk/androidTest/debug/emarsys-sdk-debug-androidTest.apk" --app=sample/build/outputs/apk/androidTest/debug/sample-debug-androidTest.apk "$FIREBASE_DEVICES" --timeout 30m --quiet --project ems-mobile-sdk &	
PIDS="$PIDS $!"
done
